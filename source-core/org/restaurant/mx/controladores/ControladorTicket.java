 ...
 * @author JAST
 */
public class ControladorTicket extends Controlador<TicketInfo> implements DatabaseAccess {
	...
	public List<TicketInfo> listarPorEstadoCortado(boolean cobrado) {
		String sql = "SELECT ticket, id_empleado, total, fecha_reg, cobrado, fecha_pagado, recibio, cambio, cortado, fechaCorte FROM TICKET WHERE CORTADO = " + cobrado;
		log.info(sql);

		Object datos[][] = conector.consultar(sql);
		List<TicketInfo> tickets;
		if (datos != null) {
			tickets = new ArrayList<>(datos.length);
